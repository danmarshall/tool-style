...
gl.shaderSource(fragmentShader, `
    ...
    void mainImage(out vec4 fragColor, in vec2 fragCoord) {
        ...
        for (int i = 0; i < buildingCount; ++i) {
            ...
            float gap = 0.02 + 0.03 * hash.x;
            
            hash = hash21(id + 0.9);
            float ballX = center - innerWidth + gap * (floor((uv.x - (center - innerWidth)) / gap) + 0.5);
            ...
            float ballMask = step(center - innerWidth, uv.x) * step(uv.x, center + innerWidth);
            color = mix(color, outlineColor, smoothstep(1.0, 0.0, ball) * ballMask);
        }
...
function render() {
    ...
    gl.uniform3f(resolutionUniform, canvas.width, canvas.height, 1.0);
